...
| 路径 | 目标频率/延迟 | 过载行为 |
|---|---:|---|
| pose/TF | 20-50 Hz，P95 `<30 ms` | 不丢最新状态，合并旧状态 |
| 2D/3D 跟踪 | 10 Hz，P95 `<50 ms` | 保留视觉事件，跳过严格重复帧 |
| 分割 | 3-5 Hz或事件触发 | 中间帧传播 mask |
| 深度与局部融合 | 3-5 Hz，P95 `<250 ms` | 关键帧推理，非关键帧传播 |
| 动态对象发布 | 10 Hz | 发布预测状态和协方差 |
| VLM/DAM 修正 | 异步，P95 `<5 s` | 同一实体请求合并、可重试 |

这些频率必须由硬件回放报告验证。单元测试只验收调度逻辑，不等价于模型性能达标。
...
- [x] **M2-05 硬件性能验收**
  - 目标：目标设备在线深度/传播综合达到 3-5 Hz，P95 `<250 ms`，显存不随帧数增长。
...
- [x] **M7-01 一键实时回放入口**
  - 目标：从带绝对时间的 image-sequence 以 5/10/15 Hz 回放，支持 dry-run、resume、stop-after 和报告目录。
...
# 使用已有深度的 G1 回放
python scripts/run_realtime_mapping.py --dataset DATASET --depth-backend precomputed --rate-hz 10
...
3. 预计算深度实时回放无 hard gate 失败；实际深度性能由目标 GPU 报告独立判定。
