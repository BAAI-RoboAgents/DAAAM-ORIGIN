...
def _realtime_workflow() -> WorkflowDefinition:
    ...
    groups = (
        ...
        G("semantic", "FastSAM / BotSort / DAM", "语义旁路不阻塞几何，但启动与最终 drain 是同步硬门。", (
            ...
            P("entity_association_policy", "实体关联策略", "--entity-association-policy", "choice", "legacy", choices=("legacy", "safe", "track_only")),
            P("entity_maximum_dimension_ratio", "安全关联最大尺寸比", "--entity-maximum-dimension-ratio", "number", 3.0, minimum=1, advanced=True),
            P("entity_maximum_observation_spread_m", "安全关联最大历史扩张 (m)", "--entity-maximum-observation-spread-m", "number", 0.75, minimum=0, advanced=True),
